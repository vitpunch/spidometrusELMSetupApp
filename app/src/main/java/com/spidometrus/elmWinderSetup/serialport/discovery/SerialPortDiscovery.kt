...
internal object SerialPortDiscovery {
            ...
            result?.let {scanResult ->
                scanResult.device?.let {
                    if (SerialPort.ignoreNoNameDeviceFlag) {
                        if (it.name != null) {
                            SerialPortDiscovery.addDevice(it)
                        }
                    } else {
                        SerialPortDiscovery.addDevice(it)
                    }
    ...
    internal fun addDevice(bluetoothDevice: BluetoothDevice) {
        if (!SerialPortDiscovery.unPairedDevicesListBD.contains(bluetoothDevice) && !SerialPortDiscovery.pairedDevicesListBD.contains(bluetoothDevice) && bluetoothDevice.name=="OBDII") {
            ...
            SerialPortDiscovery.unPairedDevicesList.add(
    ...
    */
    internal fun startBleScan() {
        ...
        discoveryStatusWithTypeCallback?.invoke(
            SerialPort.DISCOVERY_BLE, true)
    ...
    internal fun stopBleScan() {
        ...
        discoveryStatusWithTypeCallback?.invoke(
            SerialPort.DISCOVERY_BLE, false)
